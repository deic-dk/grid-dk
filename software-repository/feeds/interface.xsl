...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
		...
		xmlns:griddk="http://portal.grid.dk/0install/namespace"
		version="1.0">
	...
	<xsl:choose>
	 <xsl:when test='//zi:implementation[@main] | //zi:group[@main]'>
	  <p>This is a Zero Install feed.
             It can be run from the command-line like this:</p>
	  <pre>$ 0launch <xsl:value-of select='/zi:interface/@uri'/></pre>
	  <p>
	  Information about <b>0launch</b> and Zero Install as a whole can be found at
	  <a href='http://0install.net/injector.html'>the 0install.net web-site</a>.</p>
	 </xsl:when>
	 <xsl:otherwise>
	  <p>This is a Zero Install feed.
	  This software cannot be run as an application directly. It is a library for other programs to use.</p>
	  <p>For information about Zero Install, see <a href='http://0install.net'>0install.net</a>.</p>
	 </xsl:otherwise>
	</xsl:choose>
	</dd>
  ...
  <xsl:template mode='dl' match='/zi:interface/@uri'>
    <dt>Address</dt><dd><p><a href='{.}'><xsl:value-of select="."/></a></p></dd>
  </xsl:template>
  ...
  <xsl:template mode='dl' match='griddk:binaries'>
    <dt>Contained Binaries:</dt>
      <dd>
        <xsl:for-each select='griddk:binary'>
          <xsl:value-of select='@name'/>, 
        </xsl:for-each>
      </dd>
  </xsl:template>

<!-- this is not correct. main is for "implementation" elements, but 
     can be inherited from the closest group -->
  <xsl:template mode='dl' match='zi:group'>
    <dt>Default Binary:</dt>
      <dd>
